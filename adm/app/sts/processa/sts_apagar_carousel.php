...
if (!empty($id)) {
    ...
    $result_men_ver = "SELECT id, ordem AS ordem_result FROM sts_carousels WHERE ordem > (SELECT ordem FROM sts_carousels WHERE id='$id') ORDER BY ordem ASC";
    $resultado_men_ver = mysqli_query($conn, $result_men_ver);

    //Apagar a página
    $result_pg_del = "DELETE FROM sts_carousels WHERE id='$id'";
    ...
    if (mysqli_affected_rows($conn)) {
        ...
        if (($resultado_men_ver) AND ( $resultado_men_ver->num_rows != 0)) {
            while ($row_men_ver = mysqli_fetch_assoc($resultado_men_ver)) {
                ...
                $result_men_or = "UPDATE sts_carousels SET
        ...
        $_SESSION['msg'] = "<div class='alert alert-danger'>Erro: O carousel não foi apagado!</div>";
        $url_destino = pg . '/listar/sts_list_carousel';
        header("Location: $url_destino");
    }
} else {
