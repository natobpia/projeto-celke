...
if (!empty($id)) {
    ...
    $result_men_ver = "SELECT id, ordem AS ordem_result FROM sts_sobs_emps WHERE ordem > (SELECT ordem FROM sts_sobs_emps WHERE id='$id') ORDER BY ordem ASC";
    $resultado_men_ver = mysqli_query($conn, $result_men_ver);

    //Apagar a página
    $result_pg_del = "DELETE FROM sts_sobs_emps WHERE id='$id'";
    ...
    if (mysqli_affected_rows($conn)) {
        ...
        if (($resultado_men_ver) AND ( $resultado_men_ver->num_rows != 0)) {
            while ($row_men_ver = mysqli_fetch_assoc($resultado_men_ver)) {
                ...
                $result_men_or = "UPDATE sts_sobs_emps SET
        ...
        $_SESSION['msg'] = "<div class='alert alert-success'>Sobre empresa apagado com sucesso!</div>";
        $url_destino = pg . '/listar/sts_list_sob_emp';
        header("Location: $url_destino");
    } else {
        $_SESSION['msg'] = "<div class='alert alert-danger'>Erro: O Sobre empresa não foi apagado!</div>";
        $url_destino = pg . '/listar/sts_list_sob_emp';
        header("Location: $url_destino");
    }
} else {
