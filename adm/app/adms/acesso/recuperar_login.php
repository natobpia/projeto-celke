...
if (!empty($SendRecLog)) {
    ...
    if (!$erro) {
        //Pesquisar o usuário
        $result_user_recup = "SELECT id, nome, email, usuario, recuperar_senha FROM adms_usuarios WHERE email='" . $dados_validos['id'] . "' LIMIT 1";
        ...
        if ($resultado_user_recup and $resultado_user_recup->num_rows != 0) {
            ...
            $mensagem .= "Se você não solicitou essa alteração, nenhum ação é necessária. Sua senha permanecerá a mesma até que você ative este código.<br><br>";
            ...
            $email_destino = $row_user_recup['email'];
