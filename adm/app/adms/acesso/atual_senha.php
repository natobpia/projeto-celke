...
if (!empty($chave)) {
    ...
    if ($resultado_user_atu and $resultado_user_atu->num_rows != 0) {
        ...
        $url_destino = pg . "/acesso/recuperar_login";
...
if ($SendAtuSenha) {
    ...
    if (!$dados_validos) {
        $erro = true;
        $_SESSION['msg'] = "<div class='alert alert-danger'>Necessário preencher todos os campos para atualizar a senha!</div>";
    ...
    if (!$erro) {
        ...
        if (mysqli_affected_rows[$conn]) {
            $_SESSION['msg'] = "<div class='alert alert-danger'>Senha alterar com sucesso!</div>";
            $url_destino = pg . "/acesso/login";
            header("Location: $url_destino");
        } else {
            $_SESSION['msg'] = "<div class='alert alert-danger'>Erro ao digitar a senha!</div>";
        }
    }
}
...
<body class="text-center">
    <form class="form-signin" method="POST" action="">
        <h1 class="h3 mb-3 font-weigh-normal">Atualizar Senha</h1>
        <?php
