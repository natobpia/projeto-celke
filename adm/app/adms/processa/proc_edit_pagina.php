...
if ($SendEditPg) {
    ...
    if (!$dados_validos) {
        ...
        Necessário preencher todos os campos para editar a página!</div>";
    } else {
        //Proibir cadastro de página duplicado
        $result_paginas = "SELECT id FROM adms_paginas WHERE endereco ='" . $dados_validos['endereco'] . "' 
        AND adms_tps_pg_id='" . $dados_validos['adms_tps_pg_id'] . "' AND id <> '" . $dados['id'] . "'";
